...
class AutoTransitioner private constructor() : Thread() {
    ...
    override fun run() {
        try {
            while (!currentThread().isInterrupted) { //Loop
                ...
                sleep(100) //Sleep 50 milliseconds to minimize performance impact to the rest of your program
            }
    ...
    private fun setNewTransition(onStop: OpMode, transitionTo: String) {
        synchronized(this) {
            ...
            opModeManager =
                onStop.internalOpModeServices as OpModeManagerImpl //Store OpModeManagerImpl
        }
    }
    ...
    companion object {
        private val INSTANCE = AutoTransitioner() //Create singleton instance
